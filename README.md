...
The **Explore the disadvantages** section presents four animated experiments. Moving packets meet in a collision, jam signals spread as red waves, back-off rings count down, and successful frames enter the receiver. **Play / Pause**, **Step forward**, speed controls and a replay slider share one animation clock, so pausing freezes both the drawing and counters. Switching experiments or traffic settings starts a fresh replay; reduced-motion preferences disable automatic playback.

The first two experiments deliberately replay equal waits through the 16-attempt limit and offer a comparison with successful recovery. The priority experiment shows a routine frame beating an urgent one; changing urgency preserves both the random waits and playback position. The traffic experiment animates all 600 model ticks, using recorded arrivals, collisions, independent back-off and queue states. Small stacks show up to six waiting frames and a number gives the full queue length. Delivery counters change after the transmission completes. A quiet baseline is compared at the same model tick.

Long explanations are expandable below the animation. Collision and jam playback is slowed for visibility, and large back-off waits are compressed. Animation seconds are not network time. Average delivery delay excludes undelivered frames, and the scope remains shared half-duplex Ethernet rather than switched full-duplex Ethernet.
